...
export function createDay4(scene, camera) {
  ...
  const shop = new THREE.Mesh(new THREE.BoxGeometry(6,3,3), new THREE.MeshStandardMaterial({ color:0x222, emissive:0xffc6ff, emissiveIntensity:0.4 }));
  ...
  const fallback = new THREE.Mesh(new THREE.SphereGeometry(0.7,24,24), new THREE.MeshStandardMaterial({ color:0xc19a6b, emissive:0xc19a6b, emissiveIntensity:0.08 }));
  fallback.position.y=1.1; gift.add(fallback);

  const loader = new GLTFLoader();
  loader.load("/models/teddy.glb",(g)=>{ gift.clear(); g.scene.scale.set(0.8,0.8,0.8); gift.add(g.scene); },undefined,()=>{});
